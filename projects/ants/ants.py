...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i = 0
        target = self.place
        while type(target) != Hive:
            if len(target.bees) != 0 and i <= self.max_range and i >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            hurt = hurt + self.damage

        bee = self.place.bees[:]
        for b in bee:
            Insect.reduce_armor(b, hurt)
...
# END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        """注意，第13个问题需要override remove_from方法，但是本代码没有override，而是直接在第44-49行加上了限定条件"""
