...
def g_iter(n):
    ...
    i = 4
    while i <= n:
        numbers.append( 3 * numbers[i - 4] + 2 * numbers[i - 3] + numbers[i - 2])
        i = i + 1
    return numbers[-1]
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        remainingLast = remaining % 10

        return missing_digits(remaining) + missing_digits(remainingLast * 10 + last)
...
def count_change(total):
    ...
    def helper(change, m):
        ...
        return helper(change, m // 2) + helper(change - m , m)



    return helper(total, 2**int(log(total)))
